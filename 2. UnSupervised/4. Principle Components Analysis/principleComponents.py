# -*- coding: utf-8 -*-
"""
Created on Wed Feb  2 09:39:04 2022
...
std_milk=stdscl.fit_transform(milk)
pca=PCA()
principleComponents = pca.fit_transform(std_milk)
...
df = pd.DataFrame(principleComponents,columns=['PC1','PC2','PC3','PC4','PC5'],index=milk.index)
pca_loadings = pd.DataFrame(pca.components_.T, index=milk.columns, columns=['V1', 'V2','V3','V4','V5'] )
pca_loadings

##########################################################
milkscaled = pd.DataFrame(std_milk,columns=milk.columns, index=milk.index)
##########################################################
